...
public interface IEmpresasAod {
    ...
    @Results({
        @Result(property = "representante", column = "ifore1", one = @One(select = "bo.profesional.cesarnvf.modulos.aods.IRepresentantesAod.dato")),
        @Result(property = "subrubro", column = "ifore2", one = @One(select = "bo.profesional.cesarnvf.modulos.aods.ISubrubrosAod.dato")),
        @Result(property = "localidad", column = "ifore3", one = @One(select = "bo.profesional.cesarnvf.modulos.aods.ILocalidadesAod.dato")),
        @Result(property = "asociacion", column = "ifore4", one = @One(select = "bo.profesional.cesarnvf.modulos.aods.IAsociacionesAod.dato"))
    })
    ...
    @Results({
        @Result(property = "localidad", column = "idfore1", one = @One(select = "bo.profesional.cesarnvf.modulos.aods.ILocalidadesAod.dato"))
    })
    Empresas dato(Long id);

    @Insert("insert into empresas(idsubrubro, idlocalidad, idrepresentante, idasociacion, empresa, descripcion, tipo, direccion, telefono, celular, correo, facebook, twitter, instagram, paginaweb, otro, registrosenasag, latitud, longitud) values (#{idsubrubro}, #{idlocalidad}, #{idrepresentante}, #{idasociacion}, #{empresa}, #{descripcion}, #{tipo}, #{direccion}, #{telefono}, #{celular}, #{correo}, #{facebook}, #{twitter}, #{instagram}, #{paginaweb}, #{otro}, #{registrosenasag}, #{latitud}, #{longitud})")
    void adicionar(Empresas dato);

    @Update("update empresas set idsubrubro=#{idsubrubro}, idlocalidad=#{idlocalidad}, idrepresentante=#{idrepresentante}, idasociacion=#{idasociacion}, empresa=#{empresa}, descripcion=#{descripcion}, tipo=#{tipo}, direccion=#{direccion}, telefono=#{telefono}, celular=#{celular}, correo=#{correo}, facebook=#{facebook}, twitter=#{twitter}, instagram=#{instagram}, paginaweb=#{paginaweb}, otro=#{otro}, registrosenasag=#{registrosenasag}, latitud=#{latitud}, longitud=#{longitud} where idempresa=#{idempresa} ")
